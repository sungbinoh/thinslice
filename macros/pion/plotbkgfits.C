...
void plotmediandEdx(TH1D *h[pi::nCuts][pi::nIntTypes+1], int j, double correction, double correction_err, bool fakedata){
  ...
  for (int i = 0; i<pi::nIntTypes; ++i){
    h[pi::kAPA3][i+1]->Scale(totaldata/totalmc);
    if (i==0) htotmc_orig = (TH1D*)h[pi::kAPA3][1]->Clone(Form("htotmc_orig_%d",nc));
    else htotmc_orig->Add(h[pi::kAPA3][i+1]);
    if (i+1 == pi::kMIDp) h[pi::kAPA3][i+1]->Scale(correction);
    if (i==0) htotmc_bestfit = (TH1D*)h[pi::kAPA3][1]->Clone(Form("htotmc_bestfit_%d",nc));
    else htotmc_bestfit->Add(h[pi::kAPA3][i+1]);
    h[pi::kAPA3][i+1]->SetFillColor(colors[i]);
    h[pi::kAPA3][i+1]->SetLineWidth(0);
    hs->Add(h[pi::kAPA3][i+1]);
  }
  
  double max = TMath::Max(hs->GetMaximum(), h[pi::kAPA3][0]->GetMaximum());
  hs->SetMaximum(1.2*max);
  hs->Draw("hist");
  hs->SetTitle(h[pi::kAPA3][0]->GetTitle());
  ...
  h[pi::kAPA3][0]->SetMarkerStyle(20);
  h[pi::kAPA3][0]->Draw("same");
  ...
  leg->SetNColumns(3);
  if (fakedata){
    leg->AddEntry(h[pi::kAPA3][0],Form("%s %.0f","fake data",h[pi::kAPA3][0]->Integral()),"ple");
  }
  else{
    leg->AddEntry(h[pi::kAPA3][0],Form("%s %.0f","data",h[pi::kAPA3][0]->Integral()),"ple");
  }
  ...
  for (int i = 0; i<pi::nIntTypes; ++i){
    leg->AddEntry(h[pi::kAPA3][i+1], Form("%s %.0f",pi::intTypeName[i+1],h[pi::kAPA3][i+1]->Integral()), "f");
  }
  leg->Draw();
  ...
  TH1D *hratio_bestfit = (TH1D*)h[pi::kAPA3][0]->Clone(Form("hratio_bestfit_%d",nc));
  ...
  hratio_bestfit->Draw();
  TH1D *hratio_orig = (TH1D*)h[pi::kAPA3][0]->Clone(Form("hratio_orig_%d",nc));
  ...
  if (fakedata){
    can->Print(Form("plots/canfitmc_%s_%d_%s_%02d.png", "hmediandEdxSlice", pi::kAPA3, pi::cutName[pi::kAPA3], j));
    can->Print(Form("plots/canfitmc_%s_%d_%s_%02d.pdf", "hmediandEdxSlice", pi::kAPA3, pi::cutName[pi::kAPA3], j));
    hs->SetMaximum(8*max);
    hs->SetMinimum(1);
    pad1->SetLogy();
    can->Print(Form("plots/canlogfitmc_%s_%d_%s_%02d.png", "hmediandEdxSlice", pi::kAPA3, pi::cutName[pi::kAPA3], j));
    can->Print(Form("plots/canlogfitmc_%s_%d_%s_%02d.pdf", "hmediandEdxSlice", pi::kAPA3, pi::cutName[pi::kAPA3], j));
  }
  else{
    can->Print(Form("plots/canfit_%s_%d_%s_%02d.png", "hmediandEdxSlice", pi::kAPA3, pi::cutName[pi::kAPA3], j));
    can->Print(Form("plots/canfit_%s_%d_%s_%02d.pdf", "hmediandEdxSlice", pi::kAPA3, pi::cutName[pi::kAPA3], j));
    hs->SetMaximum(8*max);
    hs->SetMinimum(1);
    pad1->SetLogy();
    can->Print(Form("plots/canlogfit_%s_%d_%s_%02d.png", "hmediandEdxSlice", pi::kAPA3, pi::cutName[pi::kAPA3], j));
    can->Print(Form("plots/canlogfit_%s_%d_%s_%02d.pdf", "hmediandEdxSlice", pi::kAPA3, pi::cutName[pi::kAPA3], j));
  }
  ++nc;
}

void plotdaughter_michel_score(TH1D *h[pi::nCuts][pi::nIntTypes+1], int j, double correction, double correction_err, bool fakedata){
  ...
  for (int i = 0; i<pi::nIntTypes; ++i){
    ...
    if (i+1 == pi::kMIDmu || i+1 == pi::kMuon) h[pi::kAPA3][i+1]->Scale(correction);
    if (i==0) htotmc_bestfit = (TH1D*)h[pi::kAPA3][1]->Clone(Form("htotmc_bestfit_%d",nc));
    else htotmc_bestfit->Add(h[pi::kAPA3][i+1]);
    h[pi::kAPA3][i+1]->SetFillColor(colors[i]);
    h[pi::kAPA3][i+1]->SetLineWidth(0);
  ...
  if (fakedata){
    can->Print(Form("plots/canfitmc_%s_%d_%s_%02d.png", "hdaughter_michel_scoreSlice", pi::kAPA3, pi::cutName[pi::kAPA3], j));
    can->Print(Form("plots/canfitmc_%s_%d_%s_%02d.pdf", "hdaughter_michel_scoreSlice", pi::kAPA3, pi::cutName[pi::kAPA3], j));
    hs->SetMaximum(8*max);
    hs->SetMinimum(1);
    pad1->SetLogy();
    can->Print(Form("plots/canlogfitmc_%s_%d_%s_%02d.png", "hdaughter_michel_scoreSlice", pi::kAPA3, pi::cutName[pi::kAPA3], j));
    can->Print(Form("plots/canlogfitmc_%s_%d_%s_%02d.pdf", "hdaughter_michel_scoreSlice", pi::kAPA3, pi::cutName[pi::kAPA3], j));
  }
  else{
    can->Print(Form("plots/canfit_%s_%d_%s_%02d.png", "hdaughter_michel_scoreSlice", pi::kAPA3, pi::cutName[pi::kAPA3], j));
    can->Print(Form("plots/canfit_%s_%d_%s_%02d.pdf", "hdaughter_michel_scoreSlice", pi::kAPA3, pi::cutName[pi::kAPA3], j));
    hs->SetMaximum(8*max);
    hs->SetMinimum(1);
    pad1->SetLogy();
    can->Print(Form("plots/canlogfit_%s_%d_%s_%02d.png", "hdaughter_michel_scoreSlice", pi::kAPA3, pi::cutName[pi::kAPA3], j));
    can->Print(Form("plots/canlogfit_%s_%d_%s_%02d.pdf", "hdaughter_michel_scoreSlice", pi::kAPA3, pi::cutName[pi::kAPA3], j));
  }
  ++nc;
}
