...
int colors[p::nIntTypes] = {2, 3, 7, 33, 9, 46, 28, 41};
...
void plot1dslice(string name, int cut, string xtitle, string ytitle){
  ...
  TH1D *hdata[p::nthinslices][p::nCuts];
  TH1D *hmc[p::nthinslices][p::nCuts][p::nIntTypes+1];
  for (int i = 0; i < p::nCuts; ++i){
    for (int j = 0; j < p::nIntTypes+1; ++j){
...
void plot(){
  ...
  fmc = TFile::Open("../../install/bin/mcprod4a_proton.root");
  //fdata = TFile::Open("../install/bin/mcprod4a_proton.root");
  fdata = TFile::Open("../../install/bin/data_proton.root");
